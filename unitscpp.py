...
manpage = '''
Usage:  %(name)s [options] [configuration_file]
  Writes an include file that can be used for type-safe numerical
  computations with units.  The configuration_file controls the
  contents of the include file, including such things as:
  ...
  The benefit is that using strongly-typed units in numerical
  calculations can make it more likely your calculations give correct
  numerical results.

Options
-------
  -e
    Causes an empty configuration file to be written to stdout.  This
    will contain comments on how to construct the configuration file.
  -s
    Same as -e, but includes the 7 fundamental SI units to be added to
    the output along with common derived units and constants.  Feel
    free to edit this script to customize the output for your own
    needs.
'''[1:-1]
...
class U:
    ...
    def __str__(self):
        return "U(%s)" % ','.join(["%d" % int(i) for i in self.d])
    def __mul__(self, b):
    ...
    def __pow__(self, b):
        ...
        if b < 0:
            ...
            if b == -1:
                return x
            for i in range(abs(b)-1): x /= self
        else:
            x = U(self.d)
            for i in range(b-1): x *= self
        return x

def ParseCommandLine():
    program_name = sys.argv[0]
    if len(sys.argv) < 2:
        print(manpage % {"name" : program_name})
        sys.exit(0)
    try:
...
def ReadConfigFile(config_file):
    try:
        ...
        Error("Error:  couldn't read config file '%s'%s" % (config_file, nl))
    return lines
...
def GetSection(lines, StartString, EndString):
    ...
    if StartString not in lines:
        if EndString in lines:
            Error("Error:  Missing '%s' in config file%s" % (StartString, nl))
        return None
    if StartString in lines:
        start = lines.index(StartString)
        if EndString not in lines:
            Error("Error:  Missing '%s' in config file%s" % (EndString, nl))
        end = lines.index(EndString)
        if start > end:
            Error("Error:  '%s' comes before '%s' in config file%s" % \
...
def Split(line, char="="):
    ...
    '''
    if char not in line:
        Error("Error:  configuration file line missing '%s':%s%s%s" % 
            (char, nl, line, nl))
...
def ParseLine(line):
    ...
    '''
    if '=' not in line:
        Error("Error:  configuration file line missing '=':%s%s%s" % 
            (nl, line, nl))
    position = line.find("=")
    key = line[:position].strip()
    value = line[position+1:].strip()
...
def ProcessLines(lines):
    ...
    for line in lines:
        ...
        if key not in AllowedKeys:
            Error("Error:  configuration file line with bad key '%s':%s%s%s" % 
                (key, nl, line, nl))
        if key == f_unit:
...
def CheckData():
    ...
    for name in output_strings[fundamental]:
        if units.has_key(name):
            Error("Error:  '%s' is a duplicated fundamental unit%s" % (name, nl))
        else:
            units[name] = 0
    ...
    for item in output_strings[derived]:
        ...
        if units.has_key(type):
            Error("Error:  '%s' is a duplicated derived unit%s" % (type, nl))
        else:
            units[type] = 0
    ...
    for item in output_strings[constant]:
        if "=" not in item:
            Error("Error:  constant '%s' is missing '='%s" % (item, nl))
        items = item.split()
        if len(items) < 4:
            Error("Error:  constant '%s' is of improper form%s" % (item, nl))
        if items[0] == "const":
            key = items[2]
        else:
            key = items[1]
        if units.has_key(key):
            Error("Error:  '%s' is a duplicated constant%s" % (key, nl))
        else:
            units[key] = 0
...
def ConstructFundamentalUnits():
    ...
    for i in range(n):
        ...
        try:
            ...
            Error("Fundamental unit '%s' has a problem.%s" % (name, nl))
    # Store into the global dictionary for use with evaluating derived
    # units.
    if len(d) == 0:
        Error("Need at least one Unit statement in configuration file" + nl)
...
def ConstructDerivedUnits():
    ...
    for name in output_strings[Units]:
        try:
            s = "%s = U((%r))" % (name, output_strings[Units][name])
            c = compile(s, "", "single")
            eval(c)
        except Exception:
            Error("Internal error in ConstructDerivedUnits()" + nl)
    ...
    for i in range(n):
        ...
        if d.has_key(name):
            Error("Derived unit '%s' is already the name of a unit.%s" % 
                (name, nl))
        # Construct the string to compile
        s = expression
        ...
        try:
            ...
            Error("Derived unit '%s' not a valid expression.%s" % (name, nl))
        d[name] = U(result.d)
        locals()[name] = d[name]
    output_strings[Units] = d
